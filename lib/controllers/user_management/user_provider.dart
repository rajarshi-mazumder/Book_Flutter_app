...
import 'package:book_frontend/controllers/user_management/user_category_operations_mixin.dart';
...
class UserProvider extends ChangeNotifier with UserCategoryOperationsMixin {
  ...
  updateUserInterestedCategories(
      ...
      required BooksProvider booksProvider,
      required CategoriesProvider categoriesProvider}) {
    user?.interestedCategories = addUserInterestedCategoriesList(
    ...
    categoriesProvider.sortCategories(
        categoriesInterested: user?.interestedCategories);
    generateInterestedCategories(categoriesProvider: categoriesProvider);
  }

  generateInterestedCategories(
      {required CategoriesProvider categoriesProvider}) {
    if (_user?.interestedCategories != null) {
      _user?.interestedCategoriesList = generateInterestedCategoriesList(
          _user!.interestedCategories!, categoriesProvider.categoriesList);
